...
const dataToArr = data => { // Put user's drawings into the potential array
    ...
    for (let y = 1; y <= d; y ++) {
        for (let x = 0; x < d; x ++) {
            ...
            if (val === 0) {
                ...
                if (val === 255) { // If hardest possible, make it essentially infinite potential
                    vArr[pindex] = 100000;
                } else {
...
if (isMobile) { // Change error mesage
...
const setPCanvasSize = () => { // Update particle canvas size to full size
...
const testSize = () => {
    if (window.innerWidth * 0.35 < 200 || window.innerHeight < 415 || isMobile) { // If too small or the user is using a mobile device
...
let simx = initPos.left,
    simy = initPos.top;
window.onresize = () => { // Update sizes of canvas
    ...
    // Find upper left corner of the simulation
